...
import { publicChoices, validateReceipt, type EvaluationReceipt } from "./evaluationReceipt";
export type TheoryField = "culprit" | "motive" | "method" | "time";
export interface TheoryClaim { statement: string; reasoning: string; evidenceIds: string[]; choiceId?: string; }
export type TheoryDraft = Record<TheoryField, TheoryClaim>;
export interface TheorySubmission { id: number; submittedAt: string; draft: TheoryDraft; evaluation?: EvaluationReceipt; }
...
export function validateDraft(value: unknown, registered: readonly string[]): TheoryDraft {
  ...
  const entries = theoryFields.map(({ id }) => {
    ...
    if (claim.choiceId !== undefined && (typeof claim.choiceId !== "string" || (claim.choiceId !== "" && !publicChoices[id].options.some((option) => option.id === claim.choiceId)))) throw new Error("評価用結論が不正です。");
    return [id, { statement: claim.statement, reasoning: claim.reasoning, evidenceIds: [...claim.evidenceIds], ...(claim.choiceId !== undefined ? { choiceId: claim.choiceId } : {}) }];
  });
...
export function validateTheory(value: unknown, registered: readonly string[]): TheoryState {
  ...
  const submissions = value.submissions.map((entry, index): TheorySubmission => {
    ...
    if (entry.evaluation !== undefined && theoryFields.some(({ id }) => !draft[id].choiceId)) throw new Error("結論のない提出に評価が含まれています。");
    return { id: index + 1, submittedAt: entry.submittedAt, draft, ...(entry.evaluation !== undefined ? { evaluation: validateReceipt(entry.evaluation) } : {}) };
  });
...
export function submitTheory(state: TheoryState, registered: readonly string[], submittedAt: string): TheoryState {
  ...
  if (previous && theoryFields.every(({ id }) => (previous[id].choiceId ?? "") === (draft[id].choiceId ?? "") && previous[id].statement === draft[id].statement && previous[id].reasoning === draft[id].reasoning && [...previous[id].evidenceIds].sort().join("|") === [...draft[id].evidenceIds].sort().join("|"))) throw new Error("直前と同じ内容は提出済みです。推理を更新してから再提出してください。");
