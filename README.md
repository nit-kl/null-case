...
複数のデータベースに残された矛盾を横断し、証拠を組み立てるWebミステリーゲームです。このリポジトリには、CASE 001「存在しない404号室」の M06（捜査・ケースボード・通信・事件モデル提出）プロトタイプが入っています。
...
**現段階の提出は記入・証拠割当の確認と履歴保存です。真相の正誤判定・採点・エンディングは未実装です。** ネットワークへの提出や外部への送信はありません。保存v4はv1〜v3の進捗を維持して移行します。
