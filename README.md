...
複数のデータベースに残された矛盾を横断し、証拠を組み立てるWebミステリーゲームです。このリポジトリには、CASE 001「存在しない404号室」の M05（6ソース・証拠・ケースボード・通信ログ・保存復元）プロトタイプが入っています。
...
M03の保存は証拠・検索履歴を保持したまま、空のボードを持つ新形式へ自動移行します。推理提出は未実装です。
